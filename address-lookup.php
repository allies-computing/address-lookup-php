...
if (array_key_exists("input", $_GET)) {
	...
	$response = search_address($_GET['input'], $country, $page);

	// Set the header of our response
	header('Content-Type: application/json');

	echo json_encode($response);
} else {

	echo "<p>Pass a postcode or part of an address using <code>?input=nr147pz</code></p>";
}
...
function search_address($input = "", $country_code = "GB", $page = 0)
{
	...
	if ($input == "") {
		...
		$address_url = "https://ws.postcoder.com/pcw/" . $api_key . "/address/" . $country_code . "/" . urlencode($input) . "?page=" . $page . "&lines=2";
